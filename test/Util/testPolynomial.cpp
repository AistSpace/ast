...
{
    {
        // 测试基本多项式（带变量名）
        Polynomial poly1;
        errc_t err1 = poly1.parse("2x^2 + 3x - 1", "x");
        EXPECT_EQ(err1, 0);
...

    {
        // 测试基本多项式（不带变量名，默认使用x）
        Polynomial poly2;
        errc_t err2 = poly2.parse("2x^2 + 3x - 1");
        EXPECT_EQ(err2, 0);
...

    {
        // 测试无常数项的多项式（不带变量名）
        Polynomial poly3;
        errc_t err3 = poly3.parse("3x^3 + 5x");
        EXPECT_EQ(err3, 0);
...

    {
        // 测试带空格的多项式（不带变量名）
        Polynomial poly6;
        errc_t err6 = poly6.parse("  4x^3   -   2x^2   +   3x   -   5  ");
        EXPECT_EQ(err6, 0);
    ...
    }
    {
        // 测试带空格的多项式（不带变量名）
        Polynomial poly;
        errc_t err = poly.parse("  4 x ^ 3   -   2 x ^ 2   +   3 x   -   5  ");
        EXPECT_EQ(err, 0);
    ...
    }
    {
        // 测试不同变量名（需要显式指定）
        Polynomial poly7;
        errc_t err7 = poly7.parse("2y^2 + 3y + 1", "y");
        EXPECT_EQ(err7, 0);
    ...
    }
    {
        // 测试动态变量名识别
        Polynomial poly8;
        errc_t err8 = poly8.parse("2y^2 + 3y + 1");
        EXPECT_EQ(err8, 0);
    ...
    }
    {
        // 测试另一个动态变量名
        Polynomial poly9;
        errc_t err9 = poly9.parse("5z^3 - 2z + 7");
        EXPECT_EQ(err9, 0);
    ...
    }
    {
        // 测试浮点数
        Polynomial poly;
        errc_t err = poly.parse("2.5a^2 + 3a - 1.5");
        EXPECT_EQ(err, 0);
...
{
    {
        // 测试不匹配的变量名
        Polynomial poly;
        errc_t err = poly.parse("5x + z");
        EXPECT_NE(err, 0);
    }
    {
        // 测试无效的指数
        Polynomial poly;
        errc_t err = poly.parse("2.5a^2.5 + 3a - 1.5");
        EXPECT_NE(err, 0);

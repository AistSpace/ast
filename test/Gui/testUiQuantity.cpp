...
int main(int argc, char* argv[])
{
    ...
    // 连接信号和槽
    QObject::connect(uiQuantity, &UiQuantity::quantityChanged, [](const Quantity& quantity) {
        qDebug() << "Quantity changed:" << quantity.value() << QString::fromStdString(quantity.unit().name());
    });
    
    QObject::connect(testButton, &QPushButton::clicked, [uiQuantity]() {
        Quantity q = uiQuantity->quantity();
        qDebug() << "Current quantity:" << q.value() << QString::fromStdString(q.unit().name());
    });

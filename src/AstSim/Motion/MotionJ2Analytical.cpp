...
MotionJ2Analytical *MotionJ2Analytical::New()
{
    return new MotionJ2Analytical();
}


errc_t MotionJ2Analytical::makeEphemerisSpec(ScopedPtr<Ephemeris> &eph) const
{
    // 由于没有 EphemerisJ2Analytical 类，暂时使用 EphemerisLagrangeVar
    return MotionJ2Analytical::makeEphemerisSimple(eph);
}

errc_t MotionJ2Analytical::makeEphemerisSimple(ScopedPtr<Ephemeris> &eph) const
{
    errc_t rc;
    ...
    for(size_t i = 0; i < times.size(); i++){
        ...
        // 使用 J2 传播函数
        aJ2AnalyticalProp(t, gm, j2, re, state.pos(), state.vel());
...
void MotionJ2Analytical::accept(MotionProfileVisitor& visitor)
{
    visitor.visit(*this);
}
